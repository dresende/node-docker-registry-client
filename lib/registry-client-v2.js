...
function _basicAuthHeader(username, password) {
    var buffer = Buffer.from(username + ':' + password);
...
RegistryClientV2.prototype.listTags = function listTags(opts, cb) {
    ...
    vasync.pipeline({arg: this, funcs: [
        ...
        function call(_, next) {
            self._api.get({
                path: fmt('/v2/%s/tags/list' + (params.length ? "?" + params.join("&") : ""),
